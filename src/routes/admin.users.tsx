...
function UsersPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">Manage CRs, Teachers, and Admins across the university.</p>

      <Tabs defaultValue="cr" className="mt-6">
        <TabsList className="rounded-full">
          <TabsTrigger value="cr" className="rounded-full">CRs</TabsTrigger>
              ...
              {users.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-muted-foreground">
                    No {role}s yet. Click "+ Create {role.toUpperCase()}" to add one.
                  </td>
                </tr>
              )}
              {users.map((u) => (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl">Create new account</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2 text-sm">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Initial Password</label>
            <Input
              type="password"
              value={password}
