    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 animate-in fade-in duration-300">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Month view of every activity. Click any date or event for details.</p>
        </div>
        <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
          <Button variant="outline" size="icon" className="shrink-0 rounded-full hover:scale-105 transition-transform" onClick={() => setCursor(subMonths(cursor, 1))}>
          ...
          <Button variant="outline" size="icon" className="shrink-0 rounded-full hover:scale-105 transition-transform" onClick={() => setCursor(addMonths(cursor, 1))}>
            ...
            return (
              <div
                ...
                className={`min-h-28 border-b border-r border-border p-2 last:border-r-0 transition-all duration-200 ${
                  inMonth ? "bg-card hover:bg-muted/30" : "bg-muted/20 text-muted-foreground"
              ...
              >
                ...
                <div className="mt-1 space-y-1">
                  ...
                  {items.length > 3 && (
                    <div className="pl-1 text-[11px] font-medium text-primary hover:underline">+{items.length - 3} more</div>
                  )}
                </div>
              </div>
            );
      ...
      <div className="mt-6 flex flex-wrap gap-3 text-xs text-muted-foreground">
        {Object.entries(ACTIVITY_COLOR).map(([k, v]) => (
          <span key={k} className="inline-flex items-center gap-1.5 rounded-full bg-muted/40 px-2.5 py-1">
        ...
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center gap-2 text-primary font-medium text-sm">
              <CalendarIcon className="h-4 w-4" /> {selectedDay && format(selectedDay, "EEEE, MMMM d, yyyy")}
            </div>
            <DialogTitle className="font-display text-2xl">Activities for this day</DialogTitle>
            <DialogDescription>Click any activity below for full details.</DialogDescription>
          </DialogHeader>
            ...
            {activeDayActivities.map((a) => (
              ...
              >
                <div>
                  ...
                  <h4 className="mt-1 font-display text-base font-semibold group-hover:text-primary transition-colors">{a.title}</h4>
                  {a.room && (
