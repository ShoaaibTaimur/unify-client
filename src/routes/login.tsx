...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign in — UNIFY" },
      { name: "description", content: "Sign in to UNIFY as CR, Teacher, or Admin to manage academic activities." },
      { property: "og:title", content: "Sign in — UNIFY" },
      { property: "og:description", content: "Sign in to manage academic activities." },
    ],
  }),
  component: LoginPage,
});
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden overflow-hidden bg-gradient-to-br from-[color:var(--primary)] to-[color:var(--primary-deep)] p-12 text-primary-foreground lg:flex lg:flex-col lg:justify-between">
        <Logo />
        <div>
          <h2 className="font-display text-5xl leading-tight">One place for every academic activity.</h2>
          <p className="mt-4 max-w-md text-sm opacity-80">
            UNIFY brings Class Tests, Labs, Viva, Assignments, and Exams into a single premium portal
            — organized by Department, Batch, and Section.
          </p>
        </div>
        <p className="text-xs opacity-60">© {new Date().getFullYear()} UNIFY</p>
      </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <form className="mt-8 space-y-4" onSubmit={onSubmit}>
            <div className="space-y-1.5">
              ...
              <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 rounded-xl" placeholder="you@unify.edu" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Password</label>
              <Input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="h-11 rounded-xl" placeholder="••••••••" />
            </div>
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-border bg-muted/30 p-4 text-xs text-muted-foreground">
            Only accounts created by an administrator can sign in. CRs and teachers are added from the Admin panel.
          </div>
        </div>
      </div>
    </div>
