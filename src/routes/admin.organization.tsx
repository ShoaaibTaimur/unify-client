...
function OrganizationPage() {
  ...
  const createBatch = useMutation({
    ...
    onSuccess: () => { toast.success("Batch created"); setBatchName(""); qc.invalidateQueries({ queryKey: ["batches"] }); },
    onError: (e) => toast.error((e as Error).message),
  });
  ...
  const createSection = useMutation({
    ...
    onSuccess: () => { toast.success("Section created"); setSecName(""); qc.invalidateQueries({ queryKey: ["sections"] }); },
    onError: (e) => toast.error((e as Error).message),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingItem.type === "dep") return api.updateDepartment(editingItem.id, editingItem.name.trim());
      if (editingItem.type === "batch") return api.updateBatch(editingItem.id, editingItem.name.trim());
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Departments, batches, and sections — manage and edit everything in one place.
      </p>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      onClick={() => setEditingItem({ type: "dep", id: d.id, name: d.name })}
                      ...
                      <div key={b.id} className="rounded-xl border border-border bg-background/60 p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">({batchSections.length} sections)</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <Button
                              size="icon"
                              ...
                              onClick={() => setEditingItem({ type: "batch", id: b.id, name: b.name })}
                              title="Edit Batch"
                            >
                        ...
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          ...
                          {batchSections.map(s => (
                            ...
                            >
                              ...
                              <span>{s.name}</span>
                              <button
                                onClick={() => setEditingItem({ type: "sec", id: s.id, name: s.name })}
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display text-xl">
              Edit {editingItem?.type === "dep" ? "Department" : editingItem?.type === "batch" ? "Batch" : "Section"}
            </DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <Input
          ...
          <DialogFooter>
            ...
            <Button disabled={!editingItem?.name.trim() || updateMutation.isPending} onClick={() => updateMutation.mutate()}>
