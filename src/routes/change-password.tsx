...
export const Route = createFileRoute("/change-password")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Set a new password for your UNIFY account." },
      { property: "og:title", content: "Change password — UNIFY" },
      { property: "og:description", content: "Set a new password for your UNIFY account." },
    ],
  }),
...
function ChangePasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (user) setStoredSession(localStorage.getItem("unify_token")!, { ...user, mustChangePassword: false });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 shadow-card">
        ...
        <p className="mt-1 text-sm text-muted-foreground">You must set a new password before continuing.</p>

        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          <div className="space-y-1.5">
            <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Current password</label>
            <Input type="password" required value={current} onChange={(e) => setCurrent(e.target.value)} className="h-11 rounded-xl" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">New password</label>
            <Input type="password" required value={next} onChange={(e) => setNext(e.target.value)} className="h-11 rounded-xl" />
          </div>
          <div className="space-y-1.5">
